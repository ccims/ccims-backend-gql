...
type namedSyncNodeListQueryType<TSource extends CCIMSNode, TNode extends NamedSyncNode> = GraphQLFieldConfig<TSource, ResolverContext> &
{
    /**
     * Adds the basic limiting parameters (first/last/after/before) and the `name`, `description`, `createdBy` and `createdAt` filter into a given command
     ...
     */
    addParams: (cmd: LoadNamedSyncNodesCommand<TNode>, args: any) => void,
     ...
     */
    createResult: (src: TSource, context: ResolverContext, cmd: LoadNamedSyncNodesCommand<TNode>) => Promise<Page<TNode>>
};
 ...
 */
function namedSyncNodeListQuery<TSource extends CCIMSNode, TNode extends NamedSyncNode>(
    ...
    return {
        ...baseQuery,
        addParams: (cmd: LoadNamedSyncNodesCommand<TNode>, args: any) => {
...
export default namedSyncNodeListQuery;